...
require("components.savefloats")

-- better resizing
require("components.better-resize")
...
local layout_popup = awful.popup {
    ...
    ontop       = true,
    visible     = false,
    shape           = shape.rrect(beautiful.rounded),
    bg          = "#00000000"
}
    ...
    if beautiful.rounded and beautiful.rounded > 0 then
        client.connect_signal("manage", function (c, startup)
            if not c.fullscreen and not c.maximized then
                c.shape = shape.rrect(beautiful.rounded)
            end
        end)
        ...
        local function no_round_corners (c)
            ...
            elseif c.maximized then
                c.shape = nil
                --c.shape = shape.prrect(beautiful.rounded, true, true, false, false)
            else
                c.shape = shape.rrect(beautiful.rounded)
            end
        end
        ...
        beautiful.snap_shape = shape.rrect(beautiful.rounded)
    else
...
awful.spawn("bash " .. gears.filesystem.get_configuration_dir() .. "/config/autostart.sh " .. config_dir)
